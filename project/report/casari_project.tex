...
%\cite[468]{Ramanathan}
Absorbing species in this bandwidth are \ch{H2O} as water vapor, \ch{CO2} and trace gases like \ch{N2O}, chlorofluorocarbons (CFCs), \ch{CH4}.
%\cite[221]{primer}
...
% MC I forgot one fundamental thing: atmosphere emits as a blackbody in this bandwidth, this is the reason for using Planck's law as irradiance.
...
%\cite[469]{Ramanathan}
Chemical species absorbing in this bandwitdh are \ch{O3}, \ch{H2O} as water vapor, \ch{CO2} and \ch{O2}.
%\cite[218]{primer}
% MC refer to \cite[369]{Manabe} and \cite{US1976} for mixing ratios of absorbing species.
...
% MC I need to introduce the division in layers at the start of the Methods section and not as a mere transitionto the numerical formulation.
...
% MC here talk about diffusion adding the factor of 1.66 to the optical depth, cfr. \cite[469]{Ramanathan} for a superficial explanation and \cite[217]{primer} for a more elaborated description, copied from \cite[367]{Manabe}.
% MC for scattering see \cite[217]{primer}.
...
\end{figure*}
% MC put here diagram for calculation of irradiances across a layer and refer to it to describe the following equation.
...
% MC continue here, the derivation is in the appendix.

% MC correct formulae.

% MC also see if I need to use S_0 or S_0 / 4 and update formulae in TikZ images.
% MC complete shortwave_radiation_TOA after having completed shortwave_radiation.
...
% MC first I neglect effect from cloud diffusion.
...
Names of radiative properties ending with suffix -ance are generally used for rough surfaces, while suffix -ivity indicates smooth surfaces. In this work the suffix -ance is adopted. Refer to \cite[20]{Modest} for more information and to the definition of spectral absorptivity in \cite{CIE} for an example of the difference.

To derive the reflection term in equation~\ref{eq:irradiance_shortwave}, it is useful to consider a generic index $n$ with $1 \leq i \leq n \leq N - 1$ and to write the irradiances in terms of the irradiance transmitted by layer $i - 1$. The irradiance transmitted to layer $n + 1$ is
\begin{equation}
  \label{eq:irradiance_transmitted}
  \begin{split}
  E_\text{t}(n) & = \\
  & = \tau_\text{i}(n) (1 - \rho(n)) E_\text{t}(n - 1) = \\
  & = E_\text{t}(i - 1) \prod_{j = i}^{n} \tau_\text{i}(j) (1 - \rho(j))
  \end{split}
\end{equation}
and the irradiance transmitted by the TOA layer is the initial condition of the sequence:
\begin{equation}
  \label{eq:irradiance_transmitted_TOA}
  E_\text{t}(0) = \tau_\text{i}(0) (1 - \rho(0)) \ESTOA
  \quad .
\end{equation}
Multiple reflections are neglect, hence the irradiance reflected by layer $n + 1$
\begin{equation}
  \label{eq:irradiance_reflected}
  \begin{split}
    E_\text{r}(n) & = \\
    & = \rho(n + 1) E_\text{t}(n) = \\
    & = \rho(n + 1) E_\text{t}(i - 1) \prod_{j = i}^{n} \tau_\text{i}(j) (1 - \rho(j))
  \end{split}
\end{equation}
is reduced only by transmittances of the layers crossed by the radiation. The effective irradiance reflected by layer $n + 1$ and arriving to layer $i$ is
\begin{equation}
  \label{eq:irradiance_reflected_below}
  \begin{split}
  & E_{\text{r},\text{eff}}(n) = \\
  & = \tau_\text{i}(i + 1) \cdots \tau_\text{i}(n - 1) \tau_\text{i}(n) E_\text{r}(n) = \\
  & = \rho(n + 1) E_\text{t}(n) \prod_{k = i + 1}^{n} \tau_\text{i}(k) = \\
  & = \rho(n + 1) E_\text{t}(i - 1) \prod_{j = i}^{n} \tau_\text{i}(j) (1 - \rho(j)) \prod_{k = i + 1}^{n} \tau_\text{i}(k) = \\
  & = \rho(n + 1) \tau_\text{i}(i) (1 - \rho(i)) E_\text{t}(i - 1) \prod_{j = i + 1}^{n} \tau_\text{i}(j)^2 (1 - \rho(j))
  \end{split}
\end{equation}
valid only when $n > i$, otherwise $E_{\text{r},\text{eff}}(i + 1) = E_\text{r}(i + 1)$.
The total irradiation arriving to layer $i$ due to reflection of the shortwave radiation is
\begin{equation}
  \label{eq:irradiance_reflected_shortwave}
  \begin{split}
  & E_{\text{r},\text{S}}(i) = \\
  & = \sum_{n = i}^{N - 1} E_{\text{r},\text{eff}}(n + 1) = \\
  & = E_\text{r}(i + 1) + \sum_{n = i + 1}^{N - 1} E_{\text{r},\text{eff}}(n + 1) = \\
  & = \rho(i + 1) \tau_\text{i}(i) (1 - \rho(i)) E_\text{t}(i - 1) + \\
  & + \sum_{n = i + 1}^{N - 1} \bigg( \rho(n + 1) \tau_\text{i}(i) (1 - \rho(i)) E_\text{t}(i - 1) \prod_{j = i + 1}^{n} \tau_\text{i}(j)^2 (1 - \rho(j)) \bigg) = \\
  & = \tau_\text{i}(i) (1 - \rho(i)) E_\text{t}(i - 1) \Bigg( \rho(i + 1) + \\
  & + \sum_{n = i + 1}^{N - 1} \bigg( \prod_{j = i + 1}^{n} \tau_\text{i}(j)^2 (1 - \rho(j)) \bigg) \Bigg)
  \quad .
  \end{split}
\end{equation}

Irradiance absorbed directly by layer $i$ when radiation is transmitted from layer $i - 1$ is
\begin{equation}
  \label{eq:irradiance_absorbed}
  \begin{split}
  E_\text{a}(i) & = \alpha_\text{i}(i) (1 - \rho(i)) E_\text{t}(i - 1)
  = \\
  & = (1 - \tau_\text{i}(i)) (1 - \rho(i)) E_\text{t}(i - 1)
  \end{split}
\end{equation}
which is added to the irradiance absorbed by layer $i$ due to reflection from lower layers to obtain the total shortwave irradiance absorbed by layer $i$:
\begin{equation}
  \label{eq:irradiance_shortwave}
  \begin{split}
  & E_\text{S}(i) = \\
  & = E_\text{a}(i) + \alpha_\text{i}(i) E_{\text{r},\text{S}}(i) = \\
  & = (1 - \tau_\text{i}(i)) (1 - \rho(i)) E_\text{t}(i - 1) + \\
  & + (1 - \tau_\text{i}(i)) \tau_\text{i}(i) (1 - \rho(i)) E_\text{t}(i - 1) \Bigg( \rho(i + 1) + \\
  & + \sum_{n = i + 1}^{N - 1} \bigg( \prod_{j = i + 1}^{n} \tau_\text{i}(j)^2 (1 - \rho(j)) \bigg) \Bigg) = \\
  & = (1 - \tau_\text{i}(i)) (1 - \rho(i)) E_\text{t}(i - 1) \Bigg( 1 + \tau_\text{i}(i) \Bigg( \rho(i + 1) + \\
  & + \sum_{n = i + 1}^{N - 1} \bigg( \prod_{j = i + 1}^{n} \tau_\text{i}(j)^2 (1 - \rho(j)) \bigg) \Bigg) \Bigg)
  \quad .
  \end{split}
\end{equation}
