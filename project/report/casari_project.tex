...
\begin{table*}[h]
  ...
  \begin{tabular}[b]{cccp{0.5\textwidth}}
    ...
    $\mu_\text{m}$            & $\frac{\delta_\text{g} g}{P_\text{g} - \PTOA}$            &                         & Mass attenuation coefficient of the atmosphere                                           \\
    ...
    $z_0$            & \num{2000}            & \unit{\metre}                        & Arbitrary constant for normalisation of $z$                                            \\
...
\subsection{Stability} % MC maybe remove the section division.
% MC stability at ground level with respect to various number of levels.
Stability analysis of the numerical method is performed on $N$ and this is equivalent to change the spatial grid size. Powers of 2 are chosen as values of $N$ and step size is constant, obtained by dividing interval $[\deltaTOA, \delta_\text{g}]$ by $N$. Errors are evaluated as absolute differences between numerical and analytical values for each of $T(\delta_\text{g})$, $E_\text{U}(\delta_\text{g})$ and $E_\text{U}(\delta_\text{g})$. For $N \leq 4096$, errors are compatible with 0 within precision, while for greater $N$, errors increase due to error propagation. Normally this effect does not hinder results of simulations because lower values of $N$ are chosen for the model, since average dynamics could lose accuracy if thin layers are used.
\begin{figure*}[h]
  \centering
