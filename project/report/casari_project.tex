...
To derive the reflection term in equation~\ref{eq:irradiance_shortwave}, it is useful to consider a generic index $n$ with $1 \leq i \leq n \leq N - 1$ and to write the irradiances in terms of the irradiance transmitted by layer 0 to layer 1, which is the initial condition for the calculations of shortwave radiation:
\begin{equation}
  \label{eq:irradiance_transmitted_0}
  E_\text{t}(0) = \tau_\text{i}(0) (1 - \rho(0)) \ESTOA
  \quad .
\end{equation}
The irradiance transmitted to layer $n + 1$ is
\begin{equation}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
is reduced only by the transmittances of the crossed layers. The irradiance reflected by layer 1 is:
\begin{equation}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
Equation~\eqref{eq:irradiance_reflected_effective} is valid when $n > i$, otherwise $E_{\text{r},\text{eff}}(n) = E_\text{r}(n)$ and $E_{\text{r},\text{eff}}(0) = E_\text{r}(0)$.
