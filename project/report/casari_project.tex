...
\begin{table*}[h]
  ...
  \begin{tabular}[b]{cccc}
    ...
    % MC add the value I choose for z_TOA.
    \bottomrule
  \end{tabular}
\end{table*}
...
\subsection{Numerical formulation of the model}
\label{sec:Numerical formulation of the model}
...
Euler's method is used to solve the Ordinary Differential Equation (ODE) for the time-dependant temperature function.\cite[472]{Ramanathan} A solution for each atmospheric layer is calculated, hence the resulting values are triplets of temperature, altitude (i.e. proxy for the atmospheric layer) and time (i.e. simulation time). Further information on storage and plotting of data are presented in Section~\ref{sec:Plotting}.
...
\subsection{Common quantities used in atmospheric sciences}
\label{sec:Common quantities used in atmospheric sciences}
...
\begin{equation}
  ...
  \theta(t,z) = T(t,z) (\frac{P_0}{P(z)})^\frac{R_m}{c_P}
  \quad ,
\end{equation}
