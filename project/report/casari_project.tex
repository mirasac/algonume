...
\label{sec:Planck's law}
Spectral radiance emitted by a blackbody is given by Planck's law
\begin{equation}
  B_\nu (\nu, T) = 2 h c^2 \nu^3 \frac{1}{e^\frac{h c \nu}{k_B T} - 1}
  \quad ,
\end{equation}
where $\nu$ is the wavenumber following the notation used in this work (cfr. section~\ref{sec:Hypotheses and conventions}), $T$ in unit \unit{\kelvin} is the temperature of the emitting body and the other quantities are constants (cfr. table~\ref{tab:constants}).

% MC show irradiance calculation.
