...
In the longwave region $[\nu_\text{min}, \nu_\text{div}]$ the attenuation of radiation is dominated by absorption and scattering is neglected. %\cite[468]{Ramanathan}
Absorbing species in this bandwidth are \ch{H2O} present as water vapor, \ch{CO2} and trace gases like \ch{N2O}, chlorofluorocarbons (CFCs), \ch{CH4} (cfr. \cite[221]{primer}).

The radiant flux as function of altitude $\Phi_\text{L}(z)$ is derivated from the spectral integration % MC continue.
...
% MC maybe put this section before longwave and shortwave.
...
\end{equation}
where $q(z)$ is the total flux exchanged due to heat transfer, which depends on altitude $z$ also through $T(t,z)$. The total flux is the sum of three terms: $\Phi_\text{L}(z)$ and $\Phi_\text{S}(z)$ are the total irradiances of longwave and shortwave radiations, respectively, and $q_\text{C}(z)$ is the flux generated by convection.

Equation~\eqref{eq:temperature_derivation} can be rewritten using equation~\eqref{eq:hydrostatic_equilibrium} to drop the dependency on layer density. Moreover, $q_\text{C}(z)$ is neglected because it would require the resolution of equations from fluid dynamics. A numerical correction is adopted instead, which is described in section~\ref{sec:Convective adjustment}. With these considerations, the actual equation for temperature is:
\begin{equation}
  \label{eq:temperature_time_derivative}
  \pdv*{T(t,z)}{t} = -\frac{g}{c_P} \odv*{\big( \Phi_\text{L}(z) + \Phi_\text{S}(z) \big)}{P}
  \quad .
\end{equation}
...
Integrating the extinction coefficient
