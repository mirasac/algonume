...
plane-parallel layers. Altitude $z$ is used as vertical coordinate, spanning from Earth's surface at $z = \qty{0}{\metre}$ to the value corresponding to the top-of-atmosphere (TOA) %$\zTOA = \qty{}{\kilo\metre}$ % MC insert value.
. Each layer is identified by a value of $z$ corresponding to the lower bound of the layer and by its thickness. Layers are numbered starting from 0 from Earth's surface to TOA. % MC generally the opposite is done, check if my choice makes sense anyway.
...
% MC continue with simplification of equation.
% MC continue following or replacing what is written in \ref{sec:Numerical approach}.
