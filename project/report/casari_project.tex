...
Explicit evaluation of scattering of solar radiation is not performed due to the complexity of equations involved. A simpler parametrisation is adopted instead, taking advantage of the division of the atmposhere in plane-parallel layers. Layer $z$ Different reflectance, internal absorptance and internal transmittance $\tau_\text{i}$. For the layer at altitude $z$, reflectance $\rho(z)$ is assigned a priori (e.g. $A$ at the TOA, $A_\text{g}$ at ground level), internal transmittance is evaluated through equation~\eqref{eq:spectral_internal_transmittance} and internal absorptance is $1 - \tau_\text{i}$.
...
% MC continue.
In a similar way to hortwave radiation is given by:
\begin{equation}
  \label{eq:irradiance_longwave}
  E_\text{L}(z) = \int_{\nu_\text{min}}^{\nu_\text{div}} E_\text{L}(\nu, z) \odif{\nu}
  \quad ,
...
\begin{figure*}[h]
  \centering
  \includegraphics*[keepaspectratio=true,width=0.95\textwidth]{shortwave_radiation}
  \caption{} % MC complete.
...
Under the hypotheses in section~\ref{sec:Hypotheses and conventions}, shortwave irradiance at altitude $z$ is given by
\begin{equation}
  \label{eq:spectral_irradiance_shortwave}
  \begin{split}
    & E_\text{S}(\nu, z) = \\
    & = \pi B_\nu(\nu, T(t, 0)) \tau_\text{i}(\nu, 0, z) + \\
...
Vertical spatial dimension is discretised in
% MC put here the number of layers used.
layers, numbered downward starting from 0 at the TOA layer.
