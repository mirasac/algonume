...
\sisetup{group-digits=integer,separate-uncertainty,per-mode=symbol}
...
Sun's surface is approximated as an emitting blackbody. Solar radiation is considered orthogonal to the plane which represents Earth's surface in the RCM. Constant value $S_0$ for total solar irradiance is used in calculations. % MC continue.

Solar spectral irradiance is peaked in the ultraviolet (UV) and visible bandwidths, while spectral irradiance of Earth's surface is peaked in the infrared (IR) bandwidth. The difference between shapes of the distributions allows to split the spectrum at the intersection and to treat the two resulting spectra independently. The overlap between distributions is neglected.
The accuracy of the approximation can be quantified by the ratio
...
\begin{figure*}[h]
  \centering
  %\includegraphics*[keepaspectratio=true,width=\textwidth]{./figures/spectral_irradiance}
  \input{./figures/spectral_irradiance.tex}
  \caption{MC complete.}
