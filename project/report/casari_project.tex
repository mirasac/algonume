...
If density is assumed constant, equation~\eqref{eq:hydrostatic_equilibrium} can be solved easily resulting in a linear dependence of pressure $P$ on altitude $z$, % MC normalize how I refere to sections and equations.
\begin{equation*}
  P(z) = P_0 - \rho g (z - z_0)
  \quad ,
\end{equation*}
...
% MC here derivation for: non-constant density, ideal gas law and constant temperature.
If density is not constant, its expression is given by the ideal gas law (cfr. equation~\eqref{eq:ideal_gas_law}):
\begin{equation}
  \label{eq:non-constant_pressure}
  \rho = \frac{P}{R T}
  \quad .
\end{equation}
If temperature $T$ is assumed constant, equation~\eqref{eq:hydrostatic equilibrium} results in:
\begin{equation*}
  \dd P = -\frac{P g}{R T} \dd z \iff \frac{\dd P}{P} = -\frac{g}{R T} \dd z \impliedby % MC continue with primitives and final result.
\end{equation*}

% MC here derivation for: non-constant density, ideal gas law and constant lapse rate.
