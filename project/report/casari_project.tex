...
The total heat flux $q(z)$ is determined by radiative transfer and atmospheric convection. The RTE is not solved directly by the RCM, instead a two-stream approximation is adopted: radiation processes are grouped in upward and downward radiations and are treated separately. Neither the contribution to $q(z)$ due to atmospheric convection is obtained by solving the proper fluid dynamics equations, in its place a numerical correction is adopted and is described in section~\ref{sec:Convective adjustment}.
...
\begin{equation}
  ...
  \pdv*{T(t,z)}{t} = -\frac{1}{\varrho c_P} \odv*{\big( \Phi_\text{U}(z) + \Phi_\text{D}(z) \big)}{z}
  \quad ,
\end{equation}
where $\Phi_\text{U}(z)$ and $\Phi_\text{D}(z)$ are radiant fluxes of upward and downward radiations, respectively.

% MC explain how radiative fluxes are managed in a grey atmosphere.
...
% MC complete simplifying the equation.
\begin{equation}
...
% MC I need to introduce the division in layers at the start of the Methods section and not as a mere transition to the numerical formulation.
...
% MC solar diffuse radiant flux is neglected (e.g. diffusion due to clouds), hence transmittances are characterised only by the component of solar radiation normal to the layers.
...
%The irradiance reflected by layer 1 is:
%\begin{equation}
%  \label{eq:irradiance_reflected_1}
%  E_\text{r}(0) = \rho(1) E_\text{t}(0)
%  \quad .
%\end{equation}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
%Equation~\eqref{eq:irradiance_reflected_effective} is valid when $n > i$, otherwise $E_{\text{r},\text{eff}}(n) = E_\text{r}(n)$ and for the first layer $E_{\text{r},\text{eff}}(0) = E_\text{r}(0)$.
...
%  \begin{split}
%  & E_{\text{r},\text{S}}(i) = \\
%  & = \sum_{n = i}^{N - 1} E_{\text{r},\text{eff}}(n) = \\
%  & = E_\text{r}(i) + \sum_{n = i + 1}^{N - 1} E_{\text{r},\text{eff}}(n) = \\
%  & = \rho(i + 1) E_\text{t}(0) \prod_{j = 1}^{i} \tau_\text{i}(j) (1 - \rho(j)) + \sum_{n = i + 1}^{N - 1} \bigg( \\
%  & \qquad \rho(n + 1) E_\text{t}(0) \prod_{j = 1}^{n} \tau_\text{i}(j) (1 - \rho(j)) \prod_{k = i + 1}^{n} \tau_\text{i}(k) \bigg) = \\
%  & = E_\text{t}(0) \prod_{j = 1}^{i} \tau_\text{i}(j) (1 - \rho(j)) \Bigg( \rho(i + 1) + \sum_{n = i + 1}^{N - 1} \bigg( \\
%  & \qquad \rho(n + 1) \prod_{j = i + 1}^{n} \tau_\text{i}(j) (1 - \rho(j)) \prod_{k = i + 1}^{n} \tau_\text{i}(k) \bigg) \Bigg) = \\
%  & = E_\text{t}(0) \prod_{j = 1}^{i} \tau_\text{i}(j) (1 - \rho(j)) \Bigg( \rho(i + 1) + \sum_{n = i + 1}^{N - 1} \bigg( \\
%  & \qquad \rho(n + 1) \prod_{j = i + 1}^{n} \tau_\text{i}(j)^2 (1 - \rho(j)) \bigg) \Bigg)
%  \quad ,
%  \end{split}
%\end{equation}
%with the value for layer 0 equal to:
%\begin{equation}
%  \label{eq:irradiance_reflected_shortwave}
%  \begin{split}
%  & E_{\text{r},\text{S}}(0) = \\
%  & = \sum_{n = 0}^{N - 1} E_{\text{r},\text{eff}}(n) = \\
%  & = E_\text{r}(0) + \sum_{n = 1}^{N - 1} E_{\text{r},\text{eff}}(n) = \\
%  & = \rho(1) E_\text{t}(0) + \sum_{n = 1}^{N - 1} \bigg( \\
%  & \qquad \rho(n + 1) E_\text{t}(0) \prod_{j = 1}^{n} \tau_\text{i}(j) (1 - \rho(j)) \prod_{k = 1}^{n} \tau_\text{i}(k) \bigg) = \\
%  & = E_\text{t}(0) \Bigg( \rho(1) + \sum_{n = 1}^{N - 1} \bigg( \\
%  & \qquad \rho(n + 1) \prod_{j = 1}^{n} \tau_\text{i}(j)^2 (1 - \rho(j)) \bigg) \Bigg)
%  \quad .
%  \end{split}
%\end{equation}
