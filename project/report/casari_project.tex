...
A bijective relation exists between altitude $z$ and pressure $P$ (cfr. section~\ref{sec:Relation between pressure and altitude}), hence they can be used interchangeably as vertical coordinates. The origin of coordinate $z$ is set at Earth's surface and it is identified by value $z_\text{g}$ to keep generality. On the contrary, the Top-Of-Atmosphere altitude $\zTOA$ can not be defined clearly, hence this value is chosen arbitrarily. Pressure decreases with altitude from $P_\text{g}$ at ground level to value $\PTOA$, which can be calculated using equation~\ref{eq:pressure_constant_temperature} after a steady state of the temperature profile is reached.

Each layer is identified by a value of $z$ corresponding to the upper boundary of the layer and by its thickness. Layers are also numbered downward starting from 0 at the TOA layer.
...
\begin{equation}
  ...
  \quad ,
\end{equation}
where $\rho$ is the volumetric mass density of the gas. Each layer is homogenoeus in the sense that quantities affecting radiative calculations are not dependent on spatial position inside the layer (e.g. gas densities, extintion coefficients and specific heats at constant pressure $c_p$ are constant inside the layer).
The average Earth's surface considered in the model is approximated as an emitting blackbody with temperature $T_\text{earth}$. Average Bond albedo for Earth is identified with constant $\alpha$.

Solar radiation is made by parallel rays forming an angle of incidence $\varepsilon$ with the normal of the plane-parallel atmospheric layers. Sun's surface is approximated as an emitting blackbody with temperature $T_\text{sun}$. Constant value $S_0$ for total solar irradiance is used in calculations.

Solar spectral irradiance extends in the infrared (IR), visible (VIS) and ultraviolet (UV) bands, while spectral irradiance of Earth's surface is peaked in the IR band. The difference of their shapes allows the division of the spectrum at the intersection of the curves. The wavenumber of the intersection point is $\nu_\text{div}$, radiation with lesser wavenumbers is called longwave radiation, otherwise it is addressed as shortwave radiation. The two resulting spectra are treated independently and the overlap between curves is neglected. Accuracy of these approximations and value of $\nu_\text{div}$ are discussed in section~\ref{sec:Division of the electromagnetic spectrum}.
...
\begin{table*}[h]
  ...
  \begin{tabular}[b]{cccc}
    ...
    \midrule
    $R$              & \num{8.31446261815324} & \unit{\joule\per\kelvin\per\mole} &                                                                  \\
    ...
    $P_\text{g}$ & \num{1.013250e5} & \unit{\pascal} & Standard value from \cite[2]{US1976} \\
...
Intermediate layers of the atmosphere are treated as open systems where in general air parcels can move vertically, therefore temperature is determined by horizontally averaging the thermodynamic energy equation. Details on the derivation are in \cite[466]{Ramanathan}. The resulting differential equation describing temperature variation with time across layer $z$ is
\begin{equation}
  \label{eq:temperature_derivation}
  \pdv*{T(t,z)}{t} = -\frac{1}{\rho c_p} \odv*{q(z)}{z}
\end{equation}
where $q(z)$ is the total power exchanged due to heat transfer, which depends on altitude $z$ also through the temperature. In particular, $q(z) = \Phi_L(z) + \Phi_S(z) + q_C(z)$, where $\Phi_L(z)$ and $\Phi_S(z)$ are the total radiant fluxes generated by longwave and shortwave radiations, respectively, and $q_C(z)$ is the heat exchanged by convection.

Equation~\eqref{eq:temperature_derivation} can be rewritten using equation~\eqref{eq:hydrostatic_equilibrium} to drop the dependency on layer density. Moreover, $q_C(z)$ is neglected because it would require the resolution of equations from fluid dynamics. A numerical correction is adopted instead, which is described in section~\ref{sec:Convective adjustment}. With these considerations, the actual equation for temperature variation is:
\begin{equation}
  \label{eq:temperature_time_derivative}
  \pdv*{T(t,z)}{t} = -\frac{g}{c_p} \odv*{\big( \Phi_L(z) + \Phi_S(z) \big)}{P}
  \quad .
\end{equation}
...
...        ...      ...
t[N_t - 1] P[N - 1] T[N_t - 1][N - 1]
\end{verbatim}
Value \verb|N_t| is not known a priori since it is the number of temporal steps needed to reach convergence, instead \verb|N| is the chosen number of atmospheric layers.
...
\begin{equation}
  ...
  \begin{split}
    ...
    \iff & \frac{\odif{P}}{P} = - \frac{g}{R_m T} \odif{z} \impliedby \\
    \impliedby & \ln(P') \bigg|_{P_0}^{P(z)} = - \frac{g}{R_m T} z' \bigg|_{z_0}^{z} \iff \\
...
\begin{equation}
  ...
  \begin{split}
    ...
    \iff & \frac{\odif{P}}{P} = \frac{g}{R_m \Gamma} \frac{\odif{T}}{T} \impliedby \\
    \impliedby & \ln(P') \bigg|_{P_0}^{P(z)} = \frac{g}{R_m \Gamma} \ln(T') \bigg|_{T_0}^{T(z)} \iff \\
